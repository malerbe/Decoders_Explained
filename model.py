...
#######################################
# Following code is a close copy from the implementation made in the explanations.ipynb notebook
# The only difference is that we will use the StandaloneDecoder class
...
class StandaloneDecoder(nn.Module):
    def __init__(self,
                ...
                device,
                max_length):
        super(StandaloneDecoder, self).__init__()
    ...
    def make_causal_mask(self, sz):
        mask = torch.tril(torch.ones(sz, sz)).to(self.device)
        return mask.unsqueeze(0).unsqueeze(0)  # [1, 1, sz, sz]

    def forward(self, x): # enc_out = encoder output
        N, seq_length = x.shape
        positions = torch.arange(0, seq_length).expand(N, seq_length).to(self.device)
        x = self.dropout(self.word_embedding(x) + self.position_embedding(positions))

        # Génère automatiquement le masque causal
        trg_mask = self.make_causal_mask(seq_length)
